import { approveSSEFProposalPromotion } from "@/lib/ssef/promotion/service";
...
export async function POST(request: Request, { params }: RouteParams) {
  try {
    ...
    const result = await approveSSEFProposalPromotion({
      proposalId,
      actor: body.actor,
      reason: body.reason,
      note: body.note,
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Unable to approve SSEF proposal promotion.";
    const status =
      message.includes("not found") ||
      message.includes("not approvable") ||
